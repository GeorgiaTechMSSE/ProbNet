...
### modeling with dynamic factors (latent variables)
####################################################################
...
trainsteps = 40  ## the size of training data
#trainsteps = 20  ## the size of training data
predictsteps = 20  ## the size of prediction steps
#predictsteps = 10  ## the size of prediction steps
Epoch = 60      ## the window length for dynamic topological changes
...
#lag_max = 8             ## the lag in AR model. E.g.  x[t]=a1*x[t-1]+a2*x[t-2]+sigma where lag = 2


P=np.zeros((Timesteps, TotalNumNode))
P_var=np.zeros((Timesteps, TotalNumNode))
pred=np.zeros((Timesteps, TotalNumNode))
pred1=np.zeros((Timesteps, TotalNumNode))
...
err1=np.zeros((Timesteps, TotalNumNode))     # store forecast error
...
mean_sqr_err1 = np.zeros(TotalNumNode)
mean_sqr_err2 = np.zeros(TotalNumNode)
mean_sqr_err3 = np.zeros(TotalNumNode)

Data=np.zeros((trainsteps, TotalNumNode))
stdev1 = np.zeros((Timesteps, TotalNumNode))
stdev3 = np.zeros((Timesteps, TotalNumNode))
...
num_factor = TotalNumNode
#num_factor = 2
#num_factor = int(TotalNumNode/2)
...
for currstep in range(0,Timesteps,Epoch):
    ...
    #### VAR model ####
    var_model = cVAR(Data, adj=adj, bnds=[0,1])   # VAR constrained by topology
    var_model1 = DynamicFactorMQ(Data, factor_orders=lag_max, factor_multiplicities=num_factor, idiosyncratic_ar1=False)   # dynamic factor VAR
#    var_model1 = DynamicFactorMQ(Data, factors=num_factor, factor_orders=lag_max, idiosyncratic_ar1=False)   # dynamic factor VAR
    var_model2 = VAR(Data)                      # regular VAR
#    var_model3 = cVAR(Data, bnds=[0,1])         # VAR constrained by value range only
    var_model3 = DynamicFactorMQ(Data, factor_orders=lag_max, factor_multiplicities=num_factor, idiosyncratic_ar1=False)   # dynamic factor VAR with constraints
    results=var_model.fit(lag_max)
    lag_order = results.k_ar

    results1=var_model1.fit_em()
    ...
    for i in range(0,TotalNumNode):
        for j in range(0,TotalNumNode):
            for k in range(0, lag_max):
               if i != j and adj[i,j] == 0:
                   keys.append('L'+str(k+1)+'.'+var_model3.factor_names[i]+'->'+var_model3.factor_names[j])
                   values.append(0.0)
    with var_model3.fix_params(dict(zip(keys,values))):
        results3=var_model3.fit(method='bfgs')
...
#        mean_sqr_err3 += err3[t,:]**2
    ...
    #### predictions from Dynamic Factor model
    pred1[currstep+trainsteps:currstep+trainsteps+predictsteps,:]=results1.get_forecast(steps=predictsteps).predicted_mean
    stdev1[currstep+trainsteps:currstep+trainsteps+predictsteps,:]=results1.get_forecast(steps=predictsteps).se_mean
    err1[currstep+trainsteps:currstep+trainsteps+predictsteps,:]=pred1[currstep+trainsteps:currstep+trainsteps+predictsteps,:]-P[currstep+trainsteps:currstep+trainsteps+predictsteps,:]
    mean_sqr_err1=np.sum(err1**2, axis=0)
    #### predictions from Dynamic Factor topologically constrained model
    pred3[currstep+trainsteps:currstep+trainsteps+predictsteps,:]=results3.get_forecast(steps=predictsteps).predicted_mean
    stdev3[currstep+trainsteps:currstep+trainsteps+predictsteps,:]=results3.get_forecast(steps=predictsteps).se_mean
    err3[currstep+trainsteps:currstep+trainsteps+predictsteps,:]=pred3[currstep+trainsteps:currstep+trainsteps+predictsteps,:]-P[currstep+trainsteps:currstep+trainsteps+predictsteps,:]
    mean_sqr_err3=np.sum(err3**2, axis=0)
...
mean_sqr_err1 = mean_sqr_err1 / num_forecast
...
print('mean squared error (latent variable VAR): '+str(mean_sqr_err1)+' average for all nodes: '+str(mean_sqr_err1.mean()))
print('mean squared error (regular VAR): '+str(mean_sqr_err2)+' average for all nodes: '+str(mean_sqr_err2.mean()))
print('mean squared error (latent variable constrained VAR): '+str(mean_sqr_err3)+' average for all nodes: '+str(mean_sqr_err3.mean()))
...
fig, axs = plt.subplots(nrows=TotalNumNode,ncols=1,figsize=(4,4))
#fig, axs = plt.subplots(nrows=TotalNumNode,ncols=1,figsize=(6,8))
fig.suptitle("Topology constrained VAR: # of nodes="+str(TotalNumNode))
...
plt.xlabel('time')
plt.subplots_adjust(left=0.12, right=0.98, bottom=0.06, top=0.92, wspace=0, hspace=0)
...
### plot the dynamics of prediction probabilities
### Dynamic Factor VAR model
fig, axs = plt.subplots(nrows=TotalNumNode,ncols=1,figsize=(4,4))
#fig, axs = plt.subplots(nrows=TotalNumNode,ncols=1,figsize=(6,8))
fig.suptitle("Latent Variable VAR K="+str(num_factor)+", L="+str(lag_max))
#fig.suptitle("Dynamic Factor (Best-Case Fusion): # of nodes="+str(TotalNumNode))
#fig.suptitle("Dynamic Factor (Bayesian Fusion): # of nodes="+str(TotalNumNode))
j = 0
xticks = np.arange(0, Timesteps, 20)
for ax in axs.flat:
    ax.set_xlim([0, time[Timesteps-1]])
...
#    ax.plot(time[trainsteps:Timesteps], pred[trainsteps:Timesteps,j], 'c--', label="cVAR forecast")
    for currstep in range(0,Timesteps,Epoch):
        ax.plot(time[currstep+trainsteps:currstep+trainsteps+predictsteps], pred1[currstep+trainsteps:currstep+trainsteps+predictsteps,j], 'c--')
        ### standard deviation of constrained VAR
        ax.fill_between(time[currstep+trainsteps:currstep+trainsteps+predictsteps], pred1[currstep+trainsteps:currstep+trainsteps+predictsteps,j]-stdev1[currstep+trainsteps:currstep+trainsteps+predictsteps,j], \
                     pred1[currstep+trainsteps:currstep+trainsteps+predictsteps,j]+stdev1[currstep+trainsteps:currstep+trainsteps+predictsteps,j], color='gold')
...
### plot the dynamics of prediction probabilities
### Dynamic Factor Constrainted model
fig, axs = plt.subplots(nrows=TotalNumNode,ncols=1,figsize=(4,4))
#fig, axs = plt.subplots(nrows=TotalNumNode,ncols=1,figsize=(6,8))
fig.suptitle("Latent Variable Constrained VAR K="+str(num_factor)+", L="+str(lag_max))
j = 0
xticks = np.arange(0, Timesteps, 20)
for ax in axs.flat:
    ax.set_xlim([0, time[Timesteps-1]])
...
#    ax.set_xticklabels(['','','','',''])
    ...
    for currstep in range(0,Timesteps,Epoch):
        ...
        ### standard deviation
        ax.fill_between(time[currstep+trainsteps:currstep+trainsteps+predictsteps], pred3[currstep+trainsteps:currstep+trainsteps+predictsteps,j]-stdev3[currstep+trainsteps:currstep+trainsteps+predictsteps,j], \
                     pred3[currstep+trainsteps:currstep+trainsteps+predictsteps,j]+stdev3[currstep+trainsteps:currstep+trainsteps+predictsteps,j], color='gold')
